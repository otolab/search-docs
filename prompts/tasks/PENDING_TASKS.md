...
**状態**: Phase 3完了、Phase 4一部完了、Phase 5ペンディング
**最終更新**: 2025-10-30

#### 実装済みコマンド

**Phase 4: indexコマンド（完了）**
- ✅ `index status` - インデックス統計を表示（2025-10-30実装）
  - テキスト/JSON形式の出力対応
  - サーバ情報、インデックス統計、ワーカー状態表示

#### ペンディングコマンド

**Phase 5: configコマンド（一部）**
...
以下のコマンドは不要と判断し削除:
- ~~`index clean`~~ - IndexWorkerが自動処理
- ~~`config validate`~~ - サーバ起動時に自動検証
- ~~`config show`~~ - `cat`コマンドで代替可能

#### 実装の必要性

基本的な運用に必要なコマンドは全て実装済み。`config init`は便利機能として将来実装を検討。
