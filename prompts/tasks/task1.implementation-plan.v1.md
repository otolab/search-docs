# 実装計画 - search-docs初期実装

## 作業日
2025-01-27

## 目的
search-docsの基本実装を完了させ、動作可能な状態にする

## 作業方針

### 注意事項
1. ✅ 作業前に作業計画を立てる
2. ✅ 作業進捗をメモで管理する
3. ✅ コンテキスト圧縮後は関連文書を完全に再読み込み

### 参照ドキュメント
- @../AGENTS.md
- @../../docs/client-server-architecture.md
- @../../docs/data-model.md
- @../../docs/architecture.md

## 実装フェーズ

### Phase 1: 基盤パッケージ【優先度: 高】

#### 1.1 共通型定義 (packages/types)
- [x] インターフェイス定義
  - Document, Section, SearchResult
  - SearchDocsConfig
  - API Request/Response型
- [x] 型ユーティリティ
- [x] package.json設定
- [x] ビルド確認

**成果物**: `@search-docs/types` ✅ **完了**

**実装内容**:
- document.ts: Document, DocumentMetadata
- section.ts: Section, SectionMetadata
- config.ts: SearchDocsConfig, DEFAULT_CONFIG
- api.ts: API Request/Response, JSON-RPC types
- storage.ts: DocumentStorageインターフェイス
- index.ts: エクスポート

#### 1.2 DocumentStorage (packages/storage)
- [x] DocumentStorageインターフェイス実装
- [x] ファイルベースストレージ実装
  - JSON形式で保存
  - パス正規化
  - CRUD操作
  - ハッシュ計算
- [x] テスト作成（15テスト、全て通過）

**成果物**: `@search-docs/storage` ✅ **完了**

**実装内容**:
- FileStorageクラス
  - save/get/delete/list/exists
  - パス正規化（Windows/Unix互換）
  - SHA-256ハッシュ計算
- テスト: file-storage.test.ts（15テストケース）
  - CRUD操作
  - パス正規化
  - ハッシュ計算
  - エラーハンドリング

#### 1.3 DB Engine (packages/db-engine)
- [x] Pythonスキーマ定義 (schemas.py)
- [x] 埋め込みモデル統合 (embedding.py)
- [x] LanceDBワーカー (worker.py)
- [x] TypeScriptラッパー
- [x] テスト作成
- [x] Python環境セットアップ (uv sync)

**成果物**: `@search-docs/db-engine` ✅ **完了**

**参考**: `../sebas-chan/packages/db/src/python/`

**実装内容**:
- Python実装:
  - schemas.py: PyArrowスキーマ定義
  - embedding.py: Ruri埋め込みモデル (256d/768d)
  - worker.py: LanceDB JSON-RPCワーカー
- TypeScript実装:
  - index.ts: DBEngineクラス (JSON-RPC通信)
- テスト: db-engine.test.ts (接続、CRUD、検索、Dirty管理)

### Phase 2: サーバ実装【優先度: 高】

#### 2.1 設定管理 (packages/server)
- [ ] 設定ファイルローダー
- [ ] デフォルト設定
- [ ] バリデーション
- [ ] テスト作成

#### 2.2 ファイル検索
- [ ] Globパターンマッチング
- [ ] .gitignore解析
- [ ] ファイルウォッチャー統合
- [ ] テスト作成

#### 2.3 Markdown分割
- [ ] Markdownパーサー
- [ ] トークン計測 (tiktoken)
- [ ] 再帰的分割ロジック
- [ ] テスト作成

#### 2.4 サーバコア
- [ ] JSON-RPCサーバ実装
- [ ] APIハンドラ
  - search
  - getDocument
  - indexDocument
  - getStatus
  - rebuildIndex
- [ ] Dirtyワーカー
- [ ] テスト作成

**成果物**: `@search-docs/server`

### Phase 3: クライアント実装【優先度: 中】

#### 3.1 クライアントライブラリ (packages/client)
- [ ] SearchDocsClientクラス
- [ ] JSON-RPC通信
- [ ] エラーハンドリング
- [ ] テスト作成

**成果物**: `@search-docs/client`

#### 3.2 CLIツール (packages/cli)
- [ ] コマンド実装
  - server (start/stop/status)
  - search
  - index (rebuild/status/clean-dirty)
  - config (init/validate/show)
- [ ] 引数パース
- [ ] 出力フォーマット
- [ ] テスト作成

**成果物**: `@search-docs/cli`

### Phase 4: Claude Code統合【優先度: 中】

#### 4.1 MCP Server (packages/mcp-server)
- [ ] MCPプロトコル実装
- [ ] ツール定義
  - search
  - getDocument
- [ ] クライアントライブラリ統合
- [ ] テスト作成

**成果物**: `@search-docs/mcp-server`

## 実装順序

```
1. types → storage → db-engine (並行可能)
2. server (config → discovery → parser → core)
3. client
4. cli
5. mcp-server
```

## 各パッケージの依存関係

```
types (共通型)
  ↓
storage, db-engine (並行)
  ↓
server
  ↓
client
  ↓
cli, mcp-server (並行)
```

## マイルストーン

### M1: 基盤完成
- types, storage, db-engine完成
- 単体テスト完了

### M2: サーバ動作
- server実装完了
- 基本的な検索が動作
- 統合テスト完了

### M3: クライアント完成
- client, cli実装完了
- E2Eテスト完了

### M4: Claude Code統合
- mcp-server実装完了
- 実際のプロジェクトでの動作確認

## 各フェーズでのドキュメント更新

- [ ] README.mdの更新（使用方法の具体化）
- [ ] docs/配下のドキュメント更新
- [ ] 各パッケージのREADME作成

## リスク管理

### 技術的リスク
1. **LanceDB統合**: sebas-chanの実装を参考に進める
2. **トークン計測**: tiktokenの適切な統合
3. **JSON-RPC**: エラーハンドリングの適切な実装

### 対応策
- 各フェーズでテストを必ず作成
- 小さく実装して動作確認
- sebas-chanのコードを参照

## 次のアクション

1. ✅ 作業計画の作成
2. 🔲 packages/types の作成開始
3. 🔲 進捗をこのファイルで管理

---

**作成日**: 2025-01-27
**最終更新**: 2025-01-27
**状態**: 作成完了、実装開始待ち
